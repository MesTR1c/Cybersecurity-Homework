...
def leakingk(msg):
  ...
  str=hex(k)[2:]
  sign = sm2_crypt.sign(data,str) #  16进制
  #print(len(sign))
  r=sign[0:64]
  s=sign[64:128]
  r=int(r,16)
  s=int(s,16)
  dA=(findModReverse(r+s,n))*(n+k-s)%n
  rk=int(private_key,16)
  if rk==dA:
    print("leaking k leads to leaking of d 验证成功")
  else:
    print("error")
...
def reusingk(msg1,msg2):
  ...
  k=random.randint(1,n-1)
  str=hex(k)[2:]
  sign1=sm2_crypt.sign(data,str) #  16进制
  #print(len(sign))
  r1=sign1[0:64]
  s1=sign1[64:128]
  r1=int(r1,16)
  ...
  data2=msg2
  data=data2.encode()
  sign2=sm2_crypt.sign(data,str)
  r2=sign2[0:64]
  s2=sign2[64:128]
  r2=int(r2,16)
  s2=int(s2,16)
  dA=(n+s2-s1)*findModReverse((n+s1-s2+n+r1-r2),n)%n
  rk=int(private_key,16)
  if rk==dA:
    print("reusing k leads to leaking of d 验证成功")
  else:
    print("error")
...
def reusingk_diffuser(msg1,msg2):
  ...
  data2=msg2
  data=data2.encode()
  sign2=sm2_crypt_b.sign(data,str)
  r2=sign2[0:64]
  s2=sign2[64:128]
  r2=int(r2,16)
  s2=int(s2,16)
  dB=(n+k-s2)*findModReverse((s2+r2)%n,n)%n
  rk_B=int(private_key_B,16)
  if rk_B==dB:
    print("Alice got the leaking d of Bob 验证成功")
  else:
    print("error")
  dA=(n+k-s1)*findModReverse((s1+r1)%n,n)%n
  rk_A=int(private_key,16)
  if rk_A==dA:
    print("Bob got the leaking d of Alice 验证成功")
  else:
    print("error")
...
def sm2_ecdsa(msg):
  ...
  r2=int(r2,16)
  s2=int(s2,16)
  r1,s1=ed_crypt.sign(data1,k)
  #ed_crypt.verify(r1,s1,hash_data(msg))
  #print(findModReverse((r1+n+n-s1*s2%n-s1*r2%n),n))
  d=(s1*s2+n-int(hash_data(msg),16)%n)*(findModReverse((r1+n+n-s1*s2%n-s1*r2%n),n))%n
  rk=int(private_key,16)
  if rk==d:
    print("SM2 using same d and k with ECDSA leads to leaking of d 验证成功")
  else:
    print("error")

def pitfall():
  leakingk("Feng Xiangdi")
  reusingk("Feng Xiangdi","cybersecurity")
  reusingk_diffuser("Feng Xiangdi","cybersecurity")
  sm2_ecdsa("Feng Xiangdi")
pitfall()