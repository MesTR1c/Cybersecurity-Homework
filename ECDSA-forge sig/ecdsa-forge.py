...
class ecdsa(object):
    ...
    def forge_sig(self,e,r,s):
      u=random.randint(1,int(self.ecc_table["n"],16)-1)
      v=random.randint(1,int(self.ecc_table["n"],16)-1)
      n=int(self.ecc_table["n"],16)
      uG=self._kg2(u,self.ecc_table["g"])
      pub=self.public_key
      vP=self._kg(v,pub)
      R_1=self._add_point(uG,vP)
      R_1=self._convert_jacb_to_nor(R_1)
      x1=int(R_1[0:64],16)
      r_1=x1%n
      e_1=r_1*u*(self.findModReverse(v,n))%n
      s_1=r_1*(self.findModReverse(v,n))%n
      #print(len(hex(e_1)[2:]))
      return (r_1,s_1,hex(e_1)[2:])
...
print("初始消息为：",message)
r,s=ed_crypt.sign(message)
#print(r,s,p)
ed_crypt.verify(r,s,ed_crypt.hash_data(message))
r_1,s_1,e_1=ed_crypt.forge_sig(ed_crypt.hash_data(message),r,s)
ed_crypt.verify(r_1,s_1,e_1)